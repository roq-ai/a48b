...
import { getExtractedData, deleteExtractedDataById } from 'apiSdk/extracted-data';
...
function ExtractedDataListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('extracted_data', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/extracted-data/view/${id}`);
    }
  };
  ...
  return (
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        {hasAccess('extracted_data', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <Flex justifyContent="space-between" mb={4}>
            <Text as="h1" fontSize="2xl" fontWeight="bold">
              Extracted Data
            </Text>
            <NextLink href={`/extracted-data/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('extracted_data', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/extracted-data/edit/${record.id}`} passHref legacyBehavior>
                          <Button
