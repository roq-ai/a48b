...
import { getExtractedDataById } from 'apiSdk/extracted-data';
...
function ExtractedDataViewPage() {
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          <Text as="h1" fontSize="2xl" fontWeight="bold">
            Extracted Data Detail View
          </Text>
          {hasAccess('extracted_data', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/extracted-data/edit/${data?.id}`} passHref legacyBehavior>
              <Button
            ...
            <Box>
              {hasAccess('pdf_file', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  ...
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/pdf-files/view/${data?.pdf_file?.id}`}>
                      {data?.pdf_file?.file_path}
                    </Link>
                  </Text>
                </Flex>
              )}
